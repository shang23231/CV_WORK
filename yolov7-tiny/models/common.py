...
# Add attention model #########################
from models.BiFormer import BiF_Attention
...
class Concat_ATT(nn.Module):
    def __init__(self, dim, dimension=1):
        ...
        self.d = dimension
        self.att = BiF_Attention(dim)

    def forward(self, x):
        # return self.cat(x, self.d)
        return self.att(self.cat(x, self.d))
...
# add attention model ######################################################
from models.attention_CBAM import CBAMBlock
from models.BiFormer import BiF_Attention
...
###################### 添加注意力EMA到 yolov7_tiny_E_ELAN模块中 ############################
